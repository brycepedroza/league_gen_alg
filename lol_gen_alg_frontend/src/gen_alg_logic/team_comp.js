...
export default class TeamComp {
	...
	constructor(champ_info, champions=null, muation_rate=0.9){
	...
	calculate_fitness(enemy_team_comp){
		...
		*/
		...
		let fitness = avg_winrate + Math.log10(total_calculated);
	...
	static generate_team_comp(champ_info){
		...
		*/
		...
		let ids_chosen = [];
		for (let i = 0; i < 5; i++) {
			new_champion = TeamComp.make_new_champion(champ_info, ids_chosen);
